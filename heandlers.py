...
def generate_keyboard(result):
    incorrect_answers = [result + random.randint(1, 5), result - random.randint(1, 5)]
    answers = [result] + incorrect_answers
    random.shuffle(answers)

    buttons = [
        types.InlineKeyboardButton(text=str(answer), callback_data=f"captcha_answer_{answer}")
...
def referal_id(referer_id_arg=None, user_id=None):
    if user_id is None:
        raise ValueError("Необходимо передать идентификатор пользователя (user_id).")
    
    if referer_id_arg is not None:
        referer_ids[user_id] = referer_id_arg
    
    return referer_ids.get(user_id)
        ...
        if not db.user_exists(message.from_user.id):
            ...
            if str(referer_id) != "":
                if str(referer_id) != str(message.from_user.id):
                    await bot.send_message(referer_id, f'👤 У Вас новый <a href="tg://user?id={message.from_user.id}">реферал</a>, вы получите <b>{admin.updateConfig()}₽</b> после прохождения им капчи!', parse_mode="HTML")
                    problem, result = generate_captcha()
                    CAPTCHA_DATA[message.from_user.id] = result
                    keyboard = generate_keyboard(result)
                    await message.reply(f"Пройдите капчу: {problem}", reply_markup=keyboard)
                    referal_id(referer_id, message.from_user.id)       
                else:
...
@dp.callback_query_handler(lambda c: c.data.startswith('captcha_answer_'))
async def handle_captcha_answer(callback_query: types.CallbackQuery):
    ...
    if user_id in CAPTCHA_DATA:
        result = CAPTCHA_DATA[user_id]
        if answer == result:
            await bot.answer_callback_query(callback_query.id, text="Правильный ответ!")
            await bot.delete_message(callback_query.message.chat.id, callback_query.message.message_id)
            del CAPTCHA_DATA[user_id]
            referer_id = referal_id(user_id=callback_query.from_user.id)
            db.add_user(callback_query.from_user.id, referer_id)
            await bot.send_message(referer_id, f'💳 Вам начислено <b>{admin.updateConfig()}₽</b> за прохождение капчи <a href="tg://user?id={user_id}">реферала</a>!', parse_mode="HTML")
            del referer_ids[user_id]
            channels = db.channels_get()
            if not await check_sub_channels(channels, callback_query.from_user.id) or channels is None:
                await bot.send_message(callback_query.from_user.id, 'Привет <b>' + callback_query.from_user.username + '</b>, добро пожаловать! 👋')
                ...
                await asyncio.sleep(4.5)
                img = open('1.gif', 'rb')
                await bot.send_animation(callback_query.from_user.id, img, None,  caption='<b>В качестве приветствия начислили Вам бонус 😌</b>\n\n🤚Чтобы продолжить, вам нужно всего лишь подписаться на 2 канала спонсора👇🏻✨', reply_markup=nav.showChannels())
                try:
                    ...
                    balance = db.get_balance(callback_query.from_user.id)[0]
                    edit_balance = int(balance) + 500
                    db.edit_balance(edit_balance, callback_query.from_user.id)
                except Exception as e:
                    pass
            else:
                await bot.send_message(callback_query.from_user.id, 'Привет <b>' + message.from_user.username + '</b>, добро пожаловать! 👋')
                await bot.send_chat_action(callback_query.from_user.id, ChatActions.TYPING)
                await asyncio.sleep(2.5)
                await bot.send_message(callback_query.from_user.id, '➖➖➖➖➖➖➖\n💰 Ваш баланс пополнен на <b>500₽</b>\n➖➖➖➖➖➖➖')
                await bot.send_chat_action(callback_query.from_user.id, ChatActions.TYPING)
            ...
            await bot.answer_callback_query(callback_query.id, text="Неправильный ответ! Попробуйте еще раз")
...
async def geolocat(call: types.CallbackQuery):
    ...
    if call.data.split("_")[0] == 'box':
        ...
        for cc in ccs:
            ...
            btn = InlineKeyboardButton(text="💰 Перейти💰", url=cc[4])
        ...
        if id == "no":
            ...
            btnDoneSub = InlineKeyboardButton(text="◀️ Назад", callback_data="returnhome")
            keyboard.insert(btnDoneSub)
            await bot.send_message(call.from_user.id,'😔 Кажется, удача на этот раз осталась где-то рядом, но не с вами. Не расстраивайтесь!\n\nНаши розыгрыши всегда полны волнения и неожиданностей. В следующий раз вы точно сможете подарить удаче шанс улыбнуться вам. 💫\n\nПока что, не забудьте наслаждаться другими красочными моментами жизни и открывать новые возможности. Ваша удача ждет своего времени!\n\nБольшое спасибо за ваше участие! 🍀 И помните, что настоящие приключения только начинаются!', reply_markup=keyboard)
