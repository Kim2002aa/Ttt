...
admins_7 = InlineKeyboardButton(text="🔄 Экспорт", callback_data="admin_7")
...
def showCC():
    keyboard = InlineKeyboardMarkup(row_width=2)
    wrong_button_position = random.randint(0, 4)
    ccs = db.cc_get()
    for i in range(1, 6):
        if i == wrong_button_position + 1:
            btn_text = str(i) + " 🎁"
            callback_data = "box_no"
        else:
            btn_text = str(i) + " 🎁"
            callback_data = "box_" + str(ccs[0][0])
